...
function crear(req, res){
    if(connection){
        ...
        if(!persona.personaNom){
            return res.status(400).send({error: true, mensaje: "El nombre es obligatorio"});
        }

        if(!persona.personaApe){
            return res.status(400).send({error: true, mensaje: "El apellido es obligatorio"});
        }

        if(persona.personaTel && persona.personaTel.length !== 10){
...
function obtenerPersona(req, res) {
    if(connection){
        const { id } = req.params; 
        let sql = `SELECT * FROM Personal WHERE personaID = ${connection.escape(id)}`;
...
function editar(req, res) {
    if(connection){
        ...
        let sql = "UPDATE Personal set ? WHERE personaID = ?";
...
function eliminar(req, res) {
    if(connection) {
        const { id } = req.params;
        let sql = "DELETE FROM Personal WHERE personaID = ?";
